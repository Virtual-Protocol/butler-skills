...
def run_replay(state_dir: Path):
    cmd = [sys.executable, str(REPLAY), str(SKILL), "--fixture", "trade-activity-page", "--state-dir", str(state_dir)]
    for kv in ENV:
        cmd += ["--env", kv]
...
def test_one_trade_per_buy_distinct_keys(tmp_path):
    state_dir = tmp_path / "state1"
    state_dir.mkdir()
    actions = run_replay(state_dir)
...
def test_no_trade_for_sell_or_null_or_offchain(tmp_path):
...
def test_replaying_same_page_twice_records_no_new_trades(tmp_path):
    state_dir = tmp_path / "state3"
    state_dir.mkdir()
    first = run_replay(state_dir)
...
def test_seen_set_drops_oldest_id_not_an_arbitrary_one(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    try:
        duty_module = _load_copytrade_duty_module()
