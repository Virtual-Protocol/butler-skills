...
- **Trading** — spot/perp/stock, copy-trading, DCA. Toolbox rows: *Trade*, *Other people's
  trades*, *Own history*, *Owner holdings / prices*. Read
  [`butler-copytrade`](https://github.com/Virtual-Protocol/butler-skill-copytrade) for a
  published skill with both a one-off and a duty mode.
...
**Protocol-specific skills only.** The generic build → dry-run → file sequence for a
contract call is in every Butler's AGENTS.md §10; a hub skill is only for a PROTOCOL-specific
interaction (a named vault, staking contract, LP position).
